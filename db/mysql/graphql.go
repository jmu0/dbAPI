...
func (tbl *dbTable) GetColumns() {
	tbl.Columns = GetColumns(conn, tbl.getDbName(), tbl.getTableName())
}

func (tbl *dbTable) GetRelationships() {
	tbl.Relationships, err = GetRelationships(conn, tbl.getDbName(), tbl.getTableName())
}
...
func (tbl *dbTable) BuildRelationships() {
		...
		if r.Cardinality == "one-to-many" {
			...
			if relTbl, ok := dbm.tables[r.ToTable]; ok {
				...
				tbl.Type.AddFieldConfig(relName, &graphql.Field{
					Name:    relName,
					Type:    relTbl.Type,
					Resolve: resolveFuncManyToOne(r.ToTable, r.FromCols, r.ToCols),
				})
				// log.Println("DEBUG: added relationship", relName, "to", tbl.Name)
				// } else {
...
func (tbl *dbTable) BuildQuery() {
	...
	addQuery(&graphql.Field{
		...
		Args:        args,
		Resolve:     resolveFunc(tbl.getDbName(), tbl.getTableName(), tbl.Columns),
	})
...
func BuildSchema(args BuildSchemaArgs) (graphql.Schema, error) {
	...
	if len(args.Tables) == 0 { //get all db/table
		for _, db := range GetDatabaseNames(conn) {
			if !skipDb(db) {
				for _, tbl := range GetTableNames(conn, db) {
...
func args2whereSQL(args map[string]interface{}, cols []Column) string {
	var ret string
	var index int
		...
		if val, ok := value.(string); val != "*" {
			...
			if index > -1 {
				...
				if ok && strings.Contains(value.(string), "*") {
					ret += " " + key + " like '" + Escape(strings.Replace(value.(string), "*", "%", -1)) + "'"
				} else {
					switch value.(type) {
					case int:
